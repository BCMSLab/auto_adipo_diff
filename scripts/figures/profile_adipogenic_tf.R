...
(profiles %>%
  ...
  ggsave(plot = .,
         filename = 'manuscript/figures/profile_adipogenic_tf_a.png',
...
(df %>%
    ...
    theme(strip.background = element_blank(),
          ...
          panel.spacing = unit(0,"null"))) %>%
  ggsave(plot = .,
         filename = 'manuscript/figures/profile_adipogenic_tf_b.png',
         width = 10, height = 10, units = 'cm')

# signal tracks
track_signal <- read_rds('autoreg/data/data_tracks.rds')
track_signal$PPARG$`0`@data <- scale(track_signal$PPARG$`0`@data)
gene_id <- list(Pparg = 19016,
                Cebpb = 12608,
                Cidec = 14311)

sizes <- c(5, 2, 6, 2, 20, 20)

# generate figures
## PPARG
png(filename = 'manuscript/figures/profile_adipogenic_tf_c.png',
...
imap(gene_id, function(x, .y) {
  ...
  first_panel = ifelse(.y == 'Pparg', TRUE, FALSE)
...
png(filename = 'manuscript/figures/profile_adipogenic_tf_e.png',
...
imap(gene_id, function(x, .y) {
  ...
  title_add = ifelse(.y == 'Pparg', TRUE, FALSE)
...
png(filename = 'manuscript/figures/profile_adipogenic_tf_d.png',
    width = 12, height = 6, units = 'cm', res = 300)
...
png(filename = 'manuscript/figures/profile_adipogenic_tf_f.png',
